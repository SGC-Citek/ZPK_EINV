CLASS zeinv_cl_api_hoadon_vnpt DEFINITION
PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      ...
      BEGIN OF ty_request_data_hdr,
        ...
        zserialcert            TYPE string,
        billingmove            TYPE string,
        ngayphathanhhoadon     TYPE string,
        originbill             TYPE string,
        invoicedatetext        TYPE string,
...
*  lấy pdf của hoá đơn thay thế nháp
      ...
      update_reference
        ...
        EXPORTING lv_message      TYPE string,
...
CLASS ZEINV_CL_API_HOADON_VNPT IMPLEMENTATION.
  ...
  METHOD adjust_invoice.
    ...
    DATA(request_action) = VALUE zeinv_adjust_invoice_action_s1(
              ...
              xml_inv_data = format_xml_adjust_78( )
    ...
    IF lv_code = 'OK'.
      ...
      ls_zeinv_hd-ngayphathanhhoadon      = lw_cr_date.
      ...
      UPDATE zeinv_tb_hd SET check_thaythe_dieuchinh = '1'
                       WHERE uuid                    = @data-header-originuuid.
  ...
  METHOD check_data.
    ...
    data-header-originbill         = |{ request_data-header-originbill ALPHA = IN }|.
    ...
    IF request_data-header-documentdate IS NOT INITIAL.
      ...
      data-header-ngayphathanhhoadon = |{ request_data-header-ngayphathanhhoadon(4) }{ request_data-header-ngayphathanhhoadon+5(2) }{ request_data-header-ngayphathanhhoadon+8(2) }|.
    ENDIF.
  ...
  METHOD delete_nhhd_doc.

    "UPDATE CUSTOMFIELD yy1_nhhd_jei
    DATA: lt_je  TYPE TABLE FOR ACTION IMPORT i_journalentrytp~change.

    SELECT DISTINCT
     i_jlit~postingkey,
     i_jlit~accountingdocument,
     i_jlit~fiscalyear,
     i_jlit~companycode,
     i_jlit~accountingdocumentitem,
     i_jlit~\_postingkey-financialaccounttype
    FROM i_operationalacctgdocitem AS i_jlit
    WHERE i_jlit~accountingdocument = @data-header-accountingdocument
      AND i_jlit~fiscalyear         = @data-header-fiscalyear
      AND i_jlit~companycode        = @data-header-companycode
*        ORDER BY i_jlit~accountingdocument,i_jlit~fiscalyear,i_jlit~companycode,i_jlit~accountingdocumentitem
    ...
    "
    DATA: lt_glitem_change   LIKE <je>-%param-_glitems,
          ls_glitem_change   LIKE LINE OF lt_glitem_change,
          ls_glitem_control  LIKE ls_glitem_change-%control,
          lt_aritems_change  LIKE <je>-%param-_aparitems,
          ls_aritems_change  LIKE LINE OF lt_aritems_change,
          ls_aritems_control LIKE ls_aritems_change-%control.
    "
    LOOP AT lt_postingkey INTO DATA(ls_postingkey).
      IF  ls_postingkey-financialaccounttype = 'K' OR ls_postingkey-financialaccounttype = 'D'.
        ls_aritems_control-yy1_nhhd_jei  = if_abap_behv=>mk-on.
        APPEND INITIAL LINE TO <je>-%param-_aparitems  REFERENCE INTO DATA(lr_aparitems_change).
        lr_aparitems_change->yy1_nhhd_jei       = cl_abap_context_info=>get_system_date(  ).
        lr_aparitems_change->glaccountlineitem  = ls_postingkey-accountingdocumentitem.
        lr_aparitems_change->%control           = ls_aritems_control.
      ELSE.
        ls_glitem_control-yy1_nhhd_jei            = if_abap_behv=>mk-on.
        APPEND INITIAL LINE TO <je>-%param-_glitems  REFERENCE INTO DATA(lr_glitem_change).
        lr_glitem_change->yy1_nhhd_jei            = cl_abap_context_info=>get_system_date(  ).
        lr_glitem_change->glaccountlineitem       = ls_postingkey-accountingdocumentitem.
        lr_glitem_change->%control                = ls_glitem_control.
      ENDIF.
    ENDLOOP.
       ...
       && |<NDHDon>|
        ...
        && |<NMua>|
           && |<Ten><![CDATA[{ data-header-tendonvi }]]></Ten>|
            && |<MST>{ data-header-buyervatnumber }</MST>|
  ...
  METHOD format_xml_import_78.
    ...
    xml = |<DSHDon><HDon>|
       ...
       && |<TTChung>|
           ...
           && |<TGia>{ data-header-accountingexchangerate }</TGia>|
           && |<HTTToan>{ data-header-paymentmethodname }</HTTToan>|
           && |<NLap>{ data-header-invoicedatetext(4) }-{ data-header-invoicedatetext+4(2) }-{ data-header-invoicedatetext+6(2) }</NLap>|
           && |<TTKhac>|
           && |<TTin>|
                 && |<TTruong>HTTToan</TTruong>|
                 && |<KDLieu>string</KDLieu>|
       ...
       && |<NMua>|
           ...
           && |<STKNHang><![CDATA[{ data-header-bankaccount }]]></STKNHang>|
           && |<HVTNMHang><![CDATA[{ data-header-buyer }]]></HVTNMHang>|
           && |<DCTDTu><![CDATA[{ data-header-buyeremail }]]></DCTDTu>|
*           && |<MKHang>{ data-header-customer }</MKHang>|
*           && |<HVTNMHang><![CDATA[{ data-header-buyer }]]></HVTNMHang>|
       && |</NMua>|
       && |<DSHHDVu>|.
...
*& Item
  ...
  METHOD format_xml_import_hasvalue.
    xml = |<Invoices>|
       && |<SerialCert>{ data-header-zserialcert }</SerialCert>|
           && |<Inv>|
               && |<key>{ data-header-uuid }</key>|
               && |<idInv>{ data-header-zidinv }</idInv>|
               && |<signValue>{ data-header-zhasvalue }</signValue>|
           && |</Inv>|
       && |</Invoices>|.
  ENDMETHOD.
...
*& Item
  ...
  METHOD gethash_inv_token.
    DATA:
      ls_zeinv_hd   TYPE zeinv_tb_hd,
      ls_zeinv_hd_i TYPE zeinv_tb_hd_i.
    DATA: lv_einvoiceno TYPE zeinv_einvoiceno.
    TRY.
        DATA(request_import) = VALUE zeinv_get_hash_inv_with_token1(
            account      = gs_auth-account
            acpass       = gs_auth-acpass
            xml_inv_data = format_xml_import_78( )
            username     = gs_auth-username
            password     = gs_auth-password
            serial_cert  = data-header-zserialcert
            pattern      = data-header-zpattern
            serial       = data-header-zserial
        ).
        gv_vnpt_service->gethash_inv_token(
          EXPORTING
            input = request_import
            fkey  = data-header-uuid
          IMPORTING
            code            = DATA(lv_import_code)
            hasvalueinvoice = DATA(lv_hashvalue)
            idinv           = DATA(lv_idinv)
            ).
        IF lv_import_code = 'OK'.
          ...
          ENDCASE.
          data-header-zhasvalue      = lv_hashvalue.
          data-header-zidinv         = lv_idinv.
          ls_zeinv_hd-hasvalueinoice = lv_hashvalue.
          ls_zeinv_hd-idinv          = lv_idinv.
          MODIFY zeinv_tb_hd FROM @ls_zeinv_hd.
          ...
          "& Process Pulish With Token
          DATA(request_import_hasvalue) = VALUE zeinv_publish_cttwith_token_s1(
            account      = gs_auth-account
            acpass       = gs_auth-acpass
            xml_inv_data = format_xml_import_hasvalue( )
            username     = gs_auth-username
            password     = gs_auth-password
            pattern      = data-header-zpattern
            serial       = data-header-zserial
        ).
          gv_vnpt_service->publish_inv_with_token(
            EXPORTING
              input    = request_import_hasvalue
              fkey     = data-header-uuid
              zpattern = data-header-zpattern
              zserial  = data-header-zserial
            IMPORTING
              code       = DATA(lv_code)
              einvoiceno = lv_einvoiceno
              ).
  ...
  METHOD get_draft_adjust.
    ...
    TRY.
        gv_vnpt_service->adjust_invoice_no_publish_2(
           EXPORTING
             fkey  = data-header-uuid
  ...
  METHOD get_draft_replace.
    ...
    DATA(request_draft_replace) = VALUE zeinv_replace_invoice_no_publ1(
          ...
          pattern      = data-header-zpattern
          serial       = data-header-zserial
          xml_inv_data = format_xml_replace_78( )
    ).

    TRY.
        gv_vnpt_service->replace_invoice_no_publish_2(
          EXPORTING
            fkey  = data-header-uuid
  ...
  METHOD get_invoice_no.
    ...
    TRY.
        ...
        IF lv_code = 'OK'.
          ...
          ls_zeinv_hd-einvoiceno = lv_einvoiceno.
          ls_zeinv_hd-einvoiceno_long = |{ data-header-zpattern(1) }{ data-header-zserial }.{ lv_einvoiceno }|.
          ls_zeinv_hd-ngayphathanhhoadon = lv_pulishdate.
          update_reference(
          EXPORTING i_einvoice_long = |{ ls_zeinv_hd-einvoiceno_long }|
          IMPORTING lv_message      = DATA(lv_log_reference)
         ).
          ls_zeinv_hd-message_reference = lv_log_reference.
          MODIFY zeinv_tb_hd FROM @ls_zeinv_hd.
          response_data-status     = 'S'.
          response_data-message    = |Phát hành thành công hoá đơn { ls_zeinv_hd-einvoiceno_long }|.
...
*$-----------------------------------------------------------$
          ...
          TRY.
              ...
              CASE request_data-function.
                ...
                WHEN  zeinv_if_constants~func_publish.
                  ...
                  AUTHORITY-CHECK OBJECT 'ZAO_ZEINV'
                       ID 'ACTVT'    FIELD '01'
                       ID 'ZEINV_PL' FIELD lw_user.
                  IF sy-subrc <> 0.
                    response_data-status  = 'F'.
                  ...
                  "Get Invoice No
                  """""""""""""""""""""""""""""""""""""""""""""""""""""""
                WHEN  zeinv_if_constants~func_geteivno.
                ...
                WHEN zeinv_if_constants=>func_get_draft_adjust.
                  ...
                  response_data-status     = 'S'.
                  response_data-sourcetype = data-header-sourcetype.
                  response_data-sothamkhao = data-header-sothamkhao.
                  response_body = xco_cp_json=>data->from_abap( response_data )->apply( VALUE #( ( xco_cp_json=>transformation->underscore_to_camel_case )
                                                                                                 ( xco_cp_json=>transformation->boolean_to_abap_bool ) ) )->to_string( ).
                  """""""""""""""""""""""""""""""""""""""""""""""""""""""
                  "Delete Draft Replace Pdf
                  """""""""""""""""""""""""""""""""""""""""""""""""""""""
                WHEN zeinv_if_constants=>func_deldraft_adjust.
                  IF data-header-uuidstatus <> zeinv_if_constants=>status_drafted.
                    response_data-status = 'F'.
                    response_data-message = |PartnerInvoiceStringID: { data-header-uuid } Chưa tồn tại Hóa đơn nháp|.
                    RETURN.
                ...
                WHEN zeinv_if_constants=>func_get_draft_replace.
                  ...
                  "Delete Draft Replace Pdf
                  """""""""""""""""""""""""""""""""""""""""""""""""""""""
                WHEN zeinv_if_constants=>func_deldraft_replace.
                  IF data-header-uuidstatus <> zeinv_if_constants=>status_drafted.
                    response_data-status = 'F'.
                    response_data-message = |PartnerInvoiceStringID: { data-header-uuid } Chưa tồn tại Hóa đơn nháp|.
                    response_body = xco_cp_json=>data->from_abap( response_data )->apply( VALUE #( ( xco_cp_json=>transformation->underscore_to_camel_case )
                  ...
                  """""""""""""""""""""""""""""""""""""""""""""""""""""""
                  "Move billing old -> new
                  """""""""""""""""""""""""""""""""""""""""""""""""""""""
                WHEN zeinv_if_constants~func_move.
                  lw_user = sy-uname.
                  AUTHORITY-CHECK OBJECT 'ZAO_ZEINV'
                       ID 'ACTVT'    FIELD '01'
                       ID 'ZEINV_PL' FIELD lw_user.
                  IF sy-subrc <> 0.
                    response_data-status  = 'F'.
                    response_data-message = |Không có quyền để thực hiện Chuyển Hoá Đơn|.
                  ELSE.
                    IF data-header-uuidstatus  = zeinv_if_constants=>status_init.
                      move_einv(  ).
                    ELSE.
                      response_data-status  = 'F'.
                      response_data-message = |PartnerInvoiceStringID: { data-header-uuid } chưa được phát hành|.
                ...
                WHEN zeinv_if_constants~func_get_xml_adjust.
                  response_body = format_xml_adjust_78( ).

                WHEN zeinv_if_constants~func_get_xml.
                  response_body = format_xml_import_78( ).

                WHEN zeinv_if_constants~func_get_xml_replace.
                  response_body = format_xml_replace_78( ).
  ...
  METHOD move_einv.
    DATA:
      ls_zeinv_hd   TYPE zeinv_tb_hd,
      ls_zeinv_hd_i TYPE zeinv_tb_hd_i,
      lt_zeinv_hd_i TYPE TABLE OF zeinv_tb_hd_i,
      lv_pulishdate TYPE cl_abap_context_info=>ty_system_date.

    SELECT *
    FROM zeinv_tb_hd
    WHERE  ltrim( zeinv_tb_hd~billingdocument,'0' ) = ltrim( @data-header-billingmove,'0' )
    INTO TABLE @DATA(lt_hddt_old).
    ...
    CASE ls_zeinv_hd-sourcetype.
      ...
      WHEN 'FI'.
        ls_zeinv_hd-ngayhoadon     = data-header-documentdate.
    ENDCASE.
    ls_zeinv_hd-billingdocument    = |{ data-header-billingdocument ALPHA = IN }|.
    ls_zeinv_hd-old_move_billing   = |{ data-header-billingmove ALPHA = IN }|.
    ls_zeinv_hd-uuidstatus         = zeinv_if_constants~status_published.
    ls_zeinv_hd-einvoiceno         = lt_hddt_old[ 1 ]-einvoiceno.
    ls_zeinv_hd-einvoiceno_long    = |{ data-header-zpattern(1) }{ data-header-zserial }.{ lt_hddt_old[ 1 ]-einvoiceno }|.
    ls_zeinv_hd-ngayphathanhhoadon = data-header-ngayphathanhhoadon.
    ...
    response_data-status     = 'S'.
    response_data-message    = |Chuyển Hoá Đơn Thành Công { ls_zeinv_hd-einvoiceno_long }|.
    response_data-einvoiceno = data-header-einvoiceno.
    LOOP AT data-item REFERENCE INTO DATA(ls_item).
      CLEAR ls_zeinv_hd_i.
      ls_zeinv_hd_i =  CORRESPONDING #( ls_item->* ).
      ls_zeinv_hd_i-uuid = ls_zeinv_hd-uuid.
      APPEND ls_zeinv_hd_i TO lt_zeinv_hd_i.
    ENDLOOP.
    READ TABLE lt_hddt_old INTO DATA(ls_hddt_old) INDEX 1.
    UPDATE zeinv_tb_hd SET is_move = @abap_true
                     WHERE uuid    = @ls_hddt_old-uuid.
    MODIFY zeinv_tb_hd   FROM @ls_zeinv_hd.
    MODIFY zeinv_tb_hd_i FROM TABLE @lt_zeinv_hd_i.
    COMMIT WORK AND WAIT.
  ...
  METHOD replace_invoice.
    ...
    IF lv_code = 'OK'.
      ...
      UPDATE zeinv_tb_hd SET   check_thaythe_dieuchinh = '2'
                         WHERE uuid                    = @data-header-originuuid.
      COMMIT WORK AND WAIT.
    ENDIF.
  ...
  METHOD update_reference.
    ...
    SELECT DISTINCT i_jei~companycode,
               i_jei~fiscalyear,
               i_jei~accountingdocument,
               i_jei~accountingdocumentitem,
               i_jei~financialaccounttype
    FROM i_journalentryitem AS i_jei
    WHERE i_jei~ledger             = '0E'
      AND i_jei~accountingdocument = @data-header-accountingdocument
    INTO TABLE @DATA(lt_jei).
    IF sy-subrc = 0.
      SORT lt_jei BY accountingdocument accountingdocumentitem.
    ENDIF.
    ...
    LOOP AT lt_jei REFERENCE INTO DATA(ls_jei).
      <je>-accountingdocument = ls_jei->accountingdocument.
      <je>-fiscalyear         = ls_jei->fiscalyear.
      <je>-companycode        = ls_jei->companycode.
      IF ls_jei->financialaccounttype = 'K' OR ls_jei->financialaccounttype = 'D'.
        APPEND INITIAL LINE TO <je>-%param-_aparitems  REFERENCE INTO DATA(lr_aparitems_change).
        lr_aparitems_change->glaccountlineitem            = ls_jei->accountingdocumentitem.
        lr_aparitems_change->assignmentreference          = i_einvoice_long.
        lr_aparitems_change->%control-assignmentreference = if_abap_behv=>mk-on.
      ELSE.
        APPEND INITIAL LINE TO <je>-%param-_glitems  REFERENCE INTO DATA(lr_glitem_change).
        lr_glitem_change->glaccountlineitem            = ls_jei->accountingdocumentitem.
        lr_glitem_change->assignmentreference          = i_einvoice_long.
        lr_glitem_change->%control-assignmentreference = if_abap_behv=>mk-on.
      ENDIF.
    ENDLOOP.
    ...
    IF ls_failed_deep IS NOT INITIAL.
      LOOP AT ls_reported_deep-journalentry ASSIGNING FIELD-SYMBOL(<ls_report_journal>).
        CASE <ls_report_journal>-%msg->if_t100_dyn_msg~msgty.
          WHEN 'E'.
            lv_message = <ls_report_journal>-%msg->if_message~get_text( ).
        ENDCASE.
      ENDLOOP.
      ROLLBACK ENTITIES.
    ELSE.
